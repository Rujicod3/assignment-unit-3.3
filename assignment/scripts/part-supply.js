...
const maxPartsPerBox = 7;
let parts = 572;
let boxesFilled = 0;
let boxesCanBeFilled = parts >= maxPartsPerBox;
let currentPartsInBox = 0;
 while ( boxesCanBeFilled ){
  parts--;
  currentPartsInBox++;

  if (currentPartsInBox === maxPartsPerBox) {
    boxesFilled++;
    currentPartsInBox = 0;
  }
  if (currentPartsInBox + parts / maxPartsPerBox <= 1) {
    boxesCanBeFilled = false;
  }
 }
